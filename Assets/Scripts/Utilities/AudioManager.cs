    ...
    **************************************************************************************************/
    public class AudioManager : MonoBehaviour
    {
        ...
        public struct AudioItem
        {
            ...
            [Range(0f, 10f)]
            public float
                RolloffFactor; // Controls how fast the sound volume decreases as the listener moves away from the source.

            public AudioMixerGroup AudioMixerGroup;

            // AudioItem constructor initializes all properties with default or passed values.
            public AudioItem(string name, AudioClip clip, AudioMixerGroup audioMixerGroup, float volume = 1f,
        ...
        public List<AudioItem> AudioItems = new();
        ...
        public void Play(string name, Transform parent)
        {
            ...
            if (AudioItems.Exists(item => item.Name == name))
            {
                ...
                var item = AudioItems.Find(item => item.Name == name);
                // Create a new temporary GameObject with an AudioSource component, named as "(Temp)".
                var obj = new GameObject(item.Name + " (Temp)", typeof(AudioSource));
        ...
        private IEnumerator FadeAudio(AudioSource source, bool fadeIn, float duration)
        {
            ...
            if (source.volume <= 0f) source.volume = startVolume;
        }
    }
}